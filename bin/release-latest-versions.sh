#!/usr/bin/env bash
#
# Generate FluentCRM stubs for the latest versions.
#
# This script:
# 1. Fetches the latest FluentCRM versions from WordPress.org
...
OUTPUT_FILE="$ROOT_DIR/fluent-crm_versions.txt"
PLUGIN_NAME="fluent-crm"
...
# How many of the latest versions to process (default 5). Override with env.
VERSIONS_LIMIT="${VERSIONS_LIMIT:-5}"

# Update from remote repository (skipped when no 'origin' remote is configured,
# e.g. a fresh local repo that has not been pushed yet).
if git remote get-url origin >/dev/null 2>&1; then
    ...
    log_step "No 'origin' remote configured — skipping remote sync."
fi
...
# Record the full version history for reference.
log_step "Collecting versions..."
for VERSION in $VERSIONS; do
    echo "$VERSION" >> "$OUTPUT_FILE"
done

# Process only the latest N versions (the rest stay recorded in the txt file).
PROCESS_FILE="$(mktemp)"
tail -n "$VERSIONS_LIMIT" "$OUTPUT_FILE" > "$PROCESS_FILE"

# Count total versions for progress display
TOTAL_VERSIONS=$(wc -l < "$PROCESS_FILE")
CURRENT_VERSION=0

log_step "Processing the latest $TOTAL_VERSIONS of $(wc -l < "$OUTPUT_FILE") versions"
...
while IFS= read -r VERSION; do
    ...
    if git diff-index --quiet HEAD --; then
        ...
        git add .
        git commit -m "Generate stubs for FluentCRM ${VERSION}"
        git tag "v${VERSION}"
    fi
